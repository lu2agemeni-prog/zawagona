...
export default function BottomNav({ unreadCount = 0 }: { unreadCount?: number }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 z-50 h-16 w-full border-t border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md transition-colors">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.name}
              href={tab.href}
              className={`relative inline-flex flex-col items-center justify-center px-5 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 group transition-colors duration-200 ${
            ...
            >
              <div className="relative">
                <tab.icon className={`mb-1 h-6 w-6 transition-all duration-200 ${isActive ? 'text-primary-600 dark:text-primary-400 scale-110' : 'text-slate-400 dark:text-slate-500 group-hover:text-primary-600 dark:group-hover:text-primary-400'}`} />
